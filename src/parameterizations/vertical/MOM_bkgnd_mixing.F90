...
subroutine calculate_bkgnd_mixing(h, tv, N2_lay, Kd_lay, Kd_int, Kv_bkgnd, jstart, jend, nj, G, GV, US, CS)

  type(ocean_grid_type),                     intent(in)    :: G   !< Grid structure.
  type(verticalGrid_type),                   intent(in)    :: GV  !< Vertical grid structure.
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in)    :: h   !< Layer thickness [H ~> m or kg m-2].
  type(thermo_var_ptrs),                     intent(in)    :: tv  !< Thermodynamics structure.
  real, dimension(SZI_(G),SZK_(GV),nj),      intent(in)    :: N2_lay !< squared buoyancy frequency associated
                                                                  !! with layers [T-2 ~> s-2]
  real, dimension(SZI_(G),SZK_(GV),nj),      intent(out)   :: Kd_lay !< The background diapycnal diffusivity of each
                                                                  !! layer [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real, dimension(SZI_(G),SZK_(GV)+1,nj),    intent(out)   :: Kd_int !< The background diapycnal diffusivity of each
                                                                  !! interface [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real, dimension(SZI_(G),SZK_(GV)+1,nj),    intent(out)   :: Kv_bkgnd !< The background vertical viscosity at
  ...
  real, dimension(SZI_(G),nj) :: Kd_sfc     !< Surface value of the diffusivity [H Z T-1 ~> m2 s-1 or kg m-1 s-1]
  real, dimension(SZI_(G),nj) :: depth      !< Distance from surface of an interface [H ~> m or kg m-2]
  real, dimension(SZI_(G),SZK_(GV),nj) :: dz !< Height change across layers [Z ~> m]
  ...
  integer :: i, j, k, is, ie, js, je, nz, jj
  ...
  if (CS%Bryan_Lewis_diffusivity) then
    ...
    do j=jstart,jend ; jj = j - jstart + 1
      do i=is,ie
        depth_int(1) = 0.0
        do k=2,nz+1
          depth_int(k) = depth_int(k-1) + US%Z_to_m*dz(i,k-1,jj)
        enddo
        ...
        ! Update Kd and Kv.
        do K=1,nz+1
          Kv_bkgnd(i,K,jj) = GV%m2_s_to_HZ_T * Kv_col(K)
          Kd_int(i,K,jj) = GV%m2_s_to_HZ_T*Kd_col(K)
        enddo
        do k=1,nz
          Kd_lay(i,k,jj) = Kd_lay(i,k,jj) + 0.5 * GV%m2_s_to_HZ_T * (Kd_col(K) + Kd_col(K+1))
        enddo
      enddo ! i loop
    enddo
  ...
  elseif (CS%horiz_varying_background) then
    ...
    do j=jstart,jend ; jj = j - jstart + 1
      do i=is,ie
        ...
        Kd_int(i,1,jj) = (CS%bckgrnd_vdc_eq + bckgrnd_vdc_psin) + bckgrnd_vdc_psis

        if (G%geoLatT(i,j) < -10.0) then
          Kd_int(i,1,jj) = Kd_int(i,1,jj) + CS%bckgrnd_vdc1
        elseif (G%geoLatT(i,j) <= 10.0) then
          Kd_int(i,1,jj) = Kd_int(i,1,jj) + CS%bckgrnd_vdc1 * (G%geoLatT(i,j)/10.0)**2
        else
          Kd_int(i,1,jj) = Kd_int(i,1,jj) + CS%bckgrnd_vdc1
        endif
        ...
        if ( (G%geoLatT(i,j) < -1.0)  .and. (G%geoLatT(i,j) > -4.0) .and. &
          ...
          Kd_int(i,1,jj) = CS%bckgrnd_vdc_Banda
        endif
    ...
    do K=1,nz+1 ; do j=jstart,jend ; jj = j - jstart + 1 ; do i=is,ie
      Kd_int(i,K,jj) = Kd_int(i,1,jj)
      Kv_bkgnd(i,K,jj) = Kd_int(i,1,jj) * CS%prandtl_bkgnd
    enddo ; enddo ; enddo
    do k=1,nz ; do j=jstart,jend ; jj = j - jstart + 1 ; do i=is,ie
      Kd_lay(i,k,jj) = Kd_int(i,1,jj)
    enddo ; enddo ; enddo
    ...
    if (CS%Henyey_IGW_background) then
      ...
      do j=jstart,jend ; jj = j - jstart + 1
        do i=is,ie
          ...
          Kd_sfc(i,jj) = max(CS%Kd_min, CS%Kd * &
    ...
    elseif (CS%Kd_tanh_lat_fn) then
      do j=jstart,jend ; jj = j - jstart + 1
        do i=is,ie
          ...
          Kd_sfc(i,jj) = max(CS%Kd_min, CS%Kd * (1.0 + &
    ...
    if ((.not.CS%physical_OBL_scheme) .and. (CS%Kd /= CS%Kd_tot_ml)) then
      ...
      do j=jstart,jend ; jj = j - jstart + 1
        do i=is,ie ; depth(i,jj) = 0.0 ; enddo
      enddo
      do k=1,nz ; do j=jstart,jend ; jj = j - jstart + 1 ; do i=is,ie
        depth_c = depth(i,jj) + 0.5*h(i,j,k)
        if (depth_c <= CS%Hmix) then ; Kd_lay(i,k,jj) = CS%Kd_tot_ml
        elseif (depth_c >= 2.0*CS%Hmix) then ; Kd_lay(i,k,jj) = Kd_sfc(i,jj)
        else
          Kd_lay(i,k,jj) = ((Kd_sfc(i,jj) - CS%Kd_tot_ml) * I_Hmix) * depth_c + (2.0*CS%Kd_tot_ml - Kd_sfc(i,jj))
        endif

        depth(i,jj) = depth(i,jj) + h(i,j,k)
      enddo ; enddo ; enddo

    else ! There is no vertical structure to the background diffusivity.
      do k=1,nz ; do j=jstart,jend ; jj = j - jstart + 1 ; do i=is,ie
        Kd_lay(i,k,jj) = Kd_sfc(i,jj)
      enddo ; enddo ; enddo
    endif
    ...
    do j=jstart,jend ; jj = j - jstart + 1
      do i=is,ie
        Kd_int(i,1,jj) = 0.0 ; Kv_bkgnd(i,1,jj) = 0.0
        Kd_int(i,nz+1,jj) = 0.0 ; Kv_bkgnd(i,nz+1,jj) = 0.0
      enddo
    enddo
    do K=2,nz ; do j=jstart,jend ; jj = j - jstart + 1 ; do i=is,ie
      Kd_int(i,K,jj) = 0.5*(Kd_lay(i,k-1,jj) + Kd_lay(i,k,jj))
      Kv_bkgnd(i,K,jj) = Kd_int(i,K,jj) * CS%prandtl_bkgnd
    enddo ; enddo ; enddo
  endif
